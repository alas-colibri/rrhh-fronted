...
import { CreateDocumentosDto, DocumentosModel, UpdateDocumentosDto } from '@models/rrhh/documentos.model';
...
export class DocumentosHttpService {
  ...
  create(payload: CreateDocumentosDto): Observable<DocumentosModel> {
  ...
  findAll(page: number = 0, search: string = ''): Observable<DocumentosModel[]> {
    const url = this.API_URL;
  ...
  findOne(id: string): Observable<DocumentosModel> {
  ...
  update(id: string, payload: UpdateDocumentosDto): Observable<DocumentosModel> {
    const url = `${this.API_URL}/${id}`;
  ...
  remove(id: string): Observable<DocumentosModel> {
    const url = `${this.API_URL}/${id}`;
  ...
  removeAll(document: DocumentosModel[]): Observable<DocumentosModel[]> {
    const url = `${this.API_URL}/remove-all`;

    this.coreService.showLoad();
    return this.httpClient.patch<ServerResponse>(url, document).pipe(
      map((response) => {
        this.coreService.hideLoad();
        this.messageService.success(response).then();
        return response.data;
